...
function checkBlock(queryw) {
    ...
    if (readCookie("kaou") == "3") {
        placement(
            "How's it going? We want to know if you're okay. If you're just researching topics like these, you can skip this otherwise continue. We're trying our best to look out for you so we want to let you know that help is available.<br>United Kingdom: Text 'SHOUT' to 85258; SHOUT<br>United States: Dial +1-800-273-8255; the NSPL<br>Canada: Dial +1-818-831-1234; Thursday's Child<br> Australia: 1800 650 890; headspace<br>Japan: Dial 03-4550-1146 (English); TELL<br><br>We love you.",
            "fact",
            "Hey, over here!"
        );
        return;
    ...
    if (
        newquestion.includes("anxiety") ||
        newquestion.includes("suicide") ||
        newquestion.includes("eating disorder") ||
        newquestion.includes("kill myself") ||
        newquestion.includes("killing myself") ||
        newquestion.includes("sad")
    ) {
        ...
        placement(
            "Whatever you're going through, we're standing with you. <br>Get help for the mental state you're in:<br>United Kingdom: Text 'SHOUT' to 85258; SHOUT<br>United States: Dial +1-800-273-8255; the NSPL<br>Canada: Dial +1-818-831-1234; Thursday's Child<br> Australia: 1800 650 890; headspace<br>Japan: Dial 03-4550-1146 (English); TELL<br><br>You shouldn't have to suffer and stay quiet about it, whatever it is, you'll get through to see the end of it.<br>Love, Stella.",
            "fact",
            "Standing With You"
        );
        return;
    ...
    if (newquestion.includes("when is") == true) {
        ...
        if (newquestion == " eid" || newquestion == "eid") {
            var result =
                "Eid al-Fitr 2020 begins at night on Saturday 23rd May.";
            placement(result, "fact", newquestion);
            return;
        }
        if (
            newquestion == " eid al-fitr" ||
            newquestion == "eid al-fitr" ||
            newquestion == " eid fitr" ||
            newquestion == " eid al fitr"
        ) {
            var result =
                "Eid al-Fitr 2020 begins at night on Saturday 23rd May.";
        ...
        if (
            newquestion == " eid al-adha" ||
            newquestion == "eid al-adha" ||
            newquestion == " eid adha" ||
            newquestion == " eid al adha"
        ) {
            var result =
                "Eid al-Adha 2020 begins at night on Thursday 30th July. Compared to Eid al-Fitr, it lasts approximately 3 days.";
            placement(result, "fact", newquestion);
            return;
        }
    }
...
function checkWiki(question) {
    try {
        ...
        const url =
            "https://en.wikipedia.org/w/api.php?action=query&prop=extracts&exsentences=2&exlimit=2&titles=" +
            question +
            "&explaintext=1&formatversion=2&origin=*&format=json";
        Http.open("GET", url);
        Http.send();
...
function sendTime() {
    ...
    var result =
        "The current time is " + hour + ":" + minute + " in your timezone.";
    placement(result);
    return;
}
function sendDate() {
    ...
    var prefix = "th";
    if (date.slice(date.length - 1) == "3") {
        var prefix = "rd";
    }
    if (date.slice(date.length - 1) == "2") {
        var prefix = "nd";
    }
